class Booking < ApplicationRecord
  ...
  class AppointmentTimeValidator 
    ...
    def validate 
      if @booking.appointment_time
        ...
        # today and tomorrow
        bookings = @user.bookings.select { |a| a.appointment_time.midnight == @booking.appointment_time.midnight || a.appointment_time.midnight == @booking.appointment_time - 1.day || a.appointment_time.midnight == @booking.appointment_time + 1.day }
        ...
        bookings.each do |booking| 
          if @booking != booking 
            if booking.appointment_time <= @booking.appointment_time && @booking.appointment_time <= booking.end_time || @booking.appointment_time <= booking.appointment_time && booking.appointment_time <= @booking.end_time
              @booking.errors.add(:appointment_time, " is not available for requested time.")
            end
          end
        end
      end
  ...
  validate do |booking| 
    AppointmentTimeValidator.new(booking).validate
  end
end
